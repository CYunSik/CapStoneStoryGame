...
public class Hint2FInteract : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else if (isPlayerNearby && Input.GetKeyDown(KeyCode.F))
        {
            ...
            {
                if (GameManager.instance.is_2FHint == 0 && !dialogueActive)
                {
                    CheckInteractionConditions(); // 조건 충족 시 대사 시작
                }
                else if (GameManager.instance.is_2FHint == 1)
                {
    ...
    private void AdvanceDialogue()
    {
        ...
        else
        {
            ...
            if (GameManager.instance != null)
            {
                GameManager.instance.is_2FHint = 1; // 상태 업데이트
            }
        }
    }

...
public class StartScrpt : MonoBehaviour
{
    ...
    public Vector2 nextSceneStartPosition; // 다음 씬에서 캐릭터의 시작 위치
    ...
    {
        // 다음 씬에 특정 조건을 저장
        PlayerPrefs.SetInt("UseCustomPosition", 1); // 위치 설정 활성화
        PlayerPrefs.SetFloat("PlayerX", nextSceneStartPosition.x);
        PlayerPrefs.SetFloat("PlayerY", nextSceneStartPosition.y);
